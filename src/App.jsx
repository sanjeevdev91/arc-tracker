...
export default function ArcTracker() {
  ...
  // See README.md for how to swap this for Firebase/Supabase to get
  // true cross-device sync between your iPhone and desktop.
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({ routines, logs, darkMode }));
      } catch (e) { console.error('Save failed', e); }
    }, 400);
    return () => clearTimeout(saveTimer.current);
  }, [routines, logs, darkMode, loaded]);
...
function SettingsView({ darkMode, setDarkMode, onExport, confirmReset, setConfirmReset, onReset, isDesktop, routineCount }) {
  return (
    <PageWrap isDesktop={isDesktop}>
      ...
      <div style={{ marginTop: 28, padding: '18px 20px', background: 'var(--surface-alt)', borderRadius: 16 }}>
        ...
        <div style={{ fontSize: 12.5, color: 'var(--text-muted)', lineHeight: 1.6 }}>
          ...
          You're tracking {routineCount} routine{routineCount === 1 ? '' : 's'}. Your data stays synced to your account across devices.
        </div>
      </div>
    </PageWrap>
  );
